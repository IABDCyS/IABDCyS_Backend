...
export class CandidaturesController {
  ...
  @ApiOperation({
    ...
    description: 'Vérifie si l\'utilisateur courant a une candidature dans une période active'
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        candidatureActive: {
          type: 'boolean',
          description: 'Vrai si l\'utilisateur a une candidature active, faux sinon'
        },
        candidatureId: {
          type: 'string',
  ...
  async verifierCandidatureActive(@Request() req) {
    const resultat = await this.candidaturesService.verifierCandidatureActive(req.user.id);
    return resultat;
  }
  ...
  @ApiOperation({
    ...
    description: 'Retrieves all currently active application periods that candidates can apply to'
  })
  @ApiResponse({
    status: 200,
    description: 'Returns all active application periods with their associated programs',
    schema: {
      type: 'array',
      items: {
        type: 'object',
        properties: {
          id: { type: 'number', description: 'Unique identifier of the application period' },
          name: { type: 'string', description: 'Name of the application period' },
          description: { type: 'string', description: 'Detailed description of the application period' },
          startDate: { type: 'string', format: 'date-time', description: 'Start date of the application period' },
          endDate: { type: 'string', format: 'date-time', description: 'End date of the application period' },
          program: {
            type: 'object',
            properties: {
              id: { type: 'number', description: 'Unique identifier of the program' },
  ...
  @ApiResponse({ status: 200, description: 'Returns all applications for the current user' })
  async getMyApplications(@Request() req,@Query() filters:QueryCandidaturesDto,) {
    return this.candidaturesService.findAll(filters,req.user.id,req.user.role);
  }
  ...
  async createDossierAcademique(
    ...
    return await this.candidaturesService.createDossierAcademique(id, createDto, req.user);
  }
  ...
  async updateDossierAcademique(
    ...
    return await this.candidaturesService.updateDossierAcademique(id, dossierId, updateDto, req.user);
  }
  ...
  async deleteDossierAcademique(
    ...
    return await this.candidaturesService.deleteDossierAcademique(id, dossierId, req.user);
  }

  @Get(':id/dossiers-academiques')
  @UseGuards(RolesGuard)
  async getDossiersAcademiques(
    @Param('id') id: string,
    @Request() req
  ) {
  ...
  async updateInformationsPersonnelles(
    ...
    return await this.candidaturesService.updateInformationsPersonnelles(id, updateDto, req.user);
  }

  @Get(':id/mon-candidature')
  @UseGuards(RolesGuard)
  @Roles(RoleUtilisateur.CANDIDAT)
  async getMonCandidature(
    @Param('id') id: string,
    @Request() req
  ) {
  ...
  @UseGuards(RolesGuard)
  async create(@Body() createCandidatureDto: CreateCandidatureDto, @Request() req) {
  ...
  @Get(':id/notes')
  @Roles(RoleUtilisateur.ADMINISTRATEUR,RoleUtilisateur.COORDINATEUR,RoleUtilisateur.CANDIDAT)
  @UseGuards(RolesGuard)
