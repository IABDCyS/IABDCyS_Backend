...
export class StatsController {
  ...
  async getInterviewerRecentActivities(@Request() req) {
    const activities = await this.statsService.getInterviewerRecentActivities(req.user.id);
    return {
      succes: true,
      donnees: activities
    };
  ...
  async getInterviewerPeriodsOverview(@Request() req) {
    const periods = await this.statsService.getInterviewerPeriodsOverview(req.user.id);
    return {
      succes: true,
      donnees: periods
    };
